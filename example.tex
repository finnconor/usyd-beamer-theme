\documentclass[aspectratio=169, 22pt]{beamer}

%% Uncomment one of the below to use the Arial font (the university recommended font)
%% Leave commented-out if you don't have the required package

%% For MiKTeX (Linux) use this one:
% \usepackage[scaled]{helvet}
% \renewcommand{\familydefault}{\sfdefault}

%% For XeLaTeX or LuaLaTeX (Windows/Mac) use this one:
% \usepackage{fontspec}
% \setmainfont{Arial}

\usepackage{amsmath}

\title{The $\tau$ vs $\pi$ argument is really long and interesting}
\subtitle{It is mostly a notational argument}
\date{\today}
\author[Malcolm]{Malcolm Ramsay}

\usetheme{usyd}

\titlegraphic{USYDTitle}
\titlegraphicbackground{usydred}

\begin{document}

\begin{frame}
  \titlepage{}
\end{frame}

\section{Argument}
\subsection{$\tau$ is better}

\begin{frame}{Why use $\tau$ when there is $\pi$}
  \begin{block}{Theorem}
    $\tau$ is great when dealing with circles
  \end{block}

  \begin{enumerate}
    \item<1-> Fourier transforms
      \begin{equation}
        \hat f(\zeta) = \int_{-\infty}^{+\infty} f(x) e^{-2\pi ix\zeta} dx
      \end{equation}
    \item<2-> A simple pendulum
      \begin{equation}
        T \approx 2\pi \sqrt{\frac{L}{g}}
      \end{equation}
  \end{enumerate}
\end{frame}


\end{document}
